...
async def _pg_init():
    ...
    async with pool.acquire() as c:
        ...
        # TASK-029 / 需求1: llm_endpoints 表幂等自举（agp schema，CREATE IF NOT EXISTS）。
        # BIGINT identity 主键与其余 identity 表一致（D3）。失败仅告警——
        # endpoint 管理不可用时，engine 仍回退 S.* 单端点，不阻断启动。
        try:
            await c.execute(
            ...
            import logging
            logging.getLogger("core.db").warning(
                "llm_endpoints 表创建失败（降级为 S.* 单端点）: %s", e)
